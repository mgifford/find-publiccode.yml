...
def process_domain(
    ...
    if discovery.http_outcome == "success" and discovery.content:
        ...
        validation = validator.validate(
            discovery.content, file_format, source_url=discovery.final_url
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Discover and validate publiccode.yml files across government domains"
        ),
  ...
  python main.py --input eu_gov_domains.csv --output results.csv --verbose
    ...
    try:
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            ...
            futures = {
                executor.submit(
                    process_domain, domain, crawler, validator
                ): (idx + start_index, domain)
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if current_time - last_summary_time >= summary_interval:
                            ...
                            found = stats['files_discovered']
                            rate = found / max(total, 1) * 100
                            print(f"\n[{ts}] Progress Update:")
                            print(f"  Processed: {total:,} domains")
                            print(f"  Found: {found} files ({rate:.1f}%)")
                            print(
                                f"  Valid YAML: {stats['yaml_valid']}"
                                f" | Spec-compliant: {stats['spec_valid']}"
                                f" | Useful: {stats['useful']}"
                            )
        ...
        results_manager.save_checkpoint(
            ...
            start_index + len(results_manager.results),
        )
