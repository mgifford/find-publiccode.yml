...
class ResultsManager:
    ...
    def add_result(
        ...
        result = {
            ...
            "redirect_chain": (
                ";".join(discovery.redirect_chain)
                if discovery.redirect_chain else ""
            ),
        ...
        if validation:
            ...
            result.update({
                ...
                "spec_errors": (
                    ";".join(validation.spec_errors)
                    if validation.spec_errors else ""
                ),
                "spec_warnings": (
                    ";".join(validation.spec_warnings)
                    if validation.spec_warnings else ""
                ),
                "validator_exit_code": validation.validator_exit_code,
                "useful": validation.useful,
                "usefulness_score": validation.usefulness_score,
                "usefulness_issues": (
                    ";".join(validation.usefulness_issues)
                    if validation.usefulness_issues else ""
                ),
        ...
        elif discovery.http_outcome in [
            "error", "timeout", "ssl_error", "connection_error"
        ]:
            self.stats["errors"] += 1
    ...
    def print_summary(self):
        ...
        if self.stats['total_domains'] > 0:
            discovery_rate = (
                self.stats['files_discovered'] / self.stats['total_domains']
            ) * 100
            print(f"Discovery rate:            {discovery_rate:>5.2f}%")

            if self.stats['files_discovered'] > 0:
                quality_rate = (
                    self.stats['useful'] / self.stats['files_discovered']
                ) * 100
    ...
    def generate_report(self, output_dir: str = "."):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write("## Methodology\n\n")
            f.write(
                "This analysis tested each government domain"
                " for publiccode.yml files at:\n"
            )
            ...
            f.write(
                "2. **Spec Compliance** - Adherence to"
                " publiccode.yml v0.5.0 standard\n"
            )
            f.write(
                "3. **Usefulness** - Practical metadata quality assessment\n\n"
            )
            ...
            if self.stats['total_domains'] > 0:
                ...
                f.write(
                    ...
                    " of domains have publiccode.yml\n"
                )

                if self.stats['files_discovered'] > 0:
                    found = self.stats['files_discovered']
                    valid_rate = self.stats['spec_valid'] / found * 100
                    quality_rate = self.stats['useful'] / found * 100
                    f.write(
                        f"- **Validation Rate:** {valid_rate:.2f}%"
                        " of discovered files are spec-compliant\n"
                    )
                    f.write(
                        f"- **Quality Rate:** {quality_rate:.2f}%"
                        " of discovered files are high quality\n"
                    )

            f.write("\n## Limitations\n\n")
            f.write(
                "- This analysis only checks website root paths,"
                " not repository-level adoption\n"
            )
            f.write("- Absence from website does not indicate non-adoption\n")
            f.write(
                "- Network errors and timeouts may undercount"
                " actual adoption\n"
            )
            f.write(
                "- Quality assessment is based on configurable heuristics\n\n"
            )

            f.write("## Reproducibility\n\n")
            f.write(
                "All data, configuration, and code used for this"
                " analysis are available.\n"
            )
            f.write(
                "Results can be independently verified by rerunning"
                " the discovery tool.\n\n"
            )
