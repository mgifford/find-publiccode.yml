...
class PublicCodeValidator:
    ...
    def __init__(self):
        ...
        if not self.validator_available:
            logger.warning(
                ...
                "go install github.com/italia/"
                "publiccode-parser-go/v5/publiccode-parser@latest"
            )
    ...
    def validate(
        self,
        content: bytes,
        file_format: str = 'publiccode.yml',
        ...
        Args:
            ...
            file_format: File format (publiccode.yml, codemeta.json,
                code.json, contribute.json).
            source_url: Optional source URL used for error-page detection.
    ...
    def _validate_yaml_syntax(
        ...
        Args:
            content: Raw bytes of the YAML file.
            result: ValidationResult to populate in place.
            source_url: Optional URL the content was fetched from; used to
                detect error/landing pages before parsing.
        """
        try:
            # Decode content
            ...
            if source_url:
                ...
                _error_tokens = (
                    'errordocument', 'errordocument.php', '/error', '/err',
                    '/inicio/errordocument', 'login', 'forbidden', 'unauthorized',
                )
                if any(token in url_lower for token in _error_tokens):
                    result.yaml_error = (
                        f"Source URL indicates an error/landing page: {source_url}"
                    )
        ...
        try:
            ...
            try:
                ...
                if proc.returncode == 0:
                    ...
                    n_errors = len(result.spec_errors)
                    logger.debug(f"Spec validation failed: {n_errors} errors")
    ...
    def _assess_usefulness(self, result: ValidationResult):
        ...
        has_license = (
            config.USEFULNESS_CHECKS.get("has_valid_license")
            and self._check_license(data)
        )
        if has_license:
            score += 20
        else:
        ...
        has_maintenance = (
            config.USEFULNESS_CHECKS.get("has_maintenance_info")
            and self._check_maintenance(data)
        )
        if has_maintenance:
            score += 15
        else:
        ...
        has_contact = (
            config.USEFULNESS_CHECKS.get("has_contact")
            and self._check_contact(data)
        )
        if has_contact:
            score += 15
        else:
